...
| [`checkSchematicTextOverlap`](./lib/check-schematic-text-overlap.ts) | Returns a `schematic_text_overlap_warning` when two standalone text elements on the same schematic sheet and subcircuit share a position, anchor, and rotation. |
...
| [`runAllSchematicChecks`](./lib/run-all-checks.ts) | Runs schematic-layout checks for component padding, ports outside component bodies, and overlapping standalone text. |
