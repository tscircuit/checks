...
| [`checkSchematicComponentMissingSheet`](./lib/check-schematic-component-missing-sheet.ts) | Returns a `schematic_component_styling_warning` with `styling_issue_type: "missing_schematic_sheet"` for components without `schematic_sheet_id` when the circuit declares sheets. Suggests setting `schSheetName`; skips schematic group representations and circuits without sheets. |
