...
| [`checkSchematicComponentMissingSheet`](./lib/check-schematic-component-missing-sheet.ts) | Returns a `schematic_component_styling_warning` with `styling_issue_type: "missing_schematic_sheet"` for every component without `schematic_sheet_id`, including circuits with no sheets. Suggests creating a sheet when needed and setting `schSheetName`; skips schematic group representations. |
...
| [`runAllSchematicChecks`](./lib/run-all-checks.ts) | Runs schematic-layout checks (`checkSchematicComponentExcessiveVerticalPadding`, `checkSchematicComponentMissingReferenceDesignatorText`, `checkSchematicComponentPortsOutsideBody`, and `checkSchematicComponentMissingSheet`). |
