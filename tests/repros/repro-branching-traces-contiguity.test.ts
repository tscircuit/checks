import { expect, test } from "bun:test"
import type { AnyCircuitElement } from "circuit-json"
import { convertCircuitJsonToPcbSvg } from "circuit-to-svg"
import { checkTracesAreContiguous } from "lib/check-traces-are-contiguous/check-traces-are-contiguous"

/**
 * Branching traces: 3 pads on the same net connected by 3 separate pcb_trace
 * objects meeting at a junction point (0,0). trace1 has a source_trace_id,
 * trace2 and trace3 do NOT (simulating the core behavior for multi-net traces).
 *
 *   Pad A (-5,0) ---trace1---> (0,0) ---trace2---> Pad B (5,0)
 *                                 |
 *                              trace3
 *                                 |
 *                              Pad C (0,5)
 *
 * Expected: zero false positives. Currently fails because traces without
 * source_trace_id have no expected ports, and their junction endpoints
 * don't land on any pad — so they get flagged as "disconnected endpoint".
 */
const circuitJson: AnyCircuitElement[] = [
  {
    type: "pcb_board",
    pcb_board_id: "pcb_board_0",
    center: { x: 0, y: 2.5 },
    width: 14,
    height: 9,
    material: "fr4",
    thickness: 1.6,
    num_layers: 2,
  },
  {
    type: "source_trace",
    source_trace_id: "source_trace_1",
    connected_source_port_ids: [
      "source_port_a",
      "source_port_b",
      "source_port_c",
    ],
    connected_source_net_ids: [],
  },
  {
    type: "pcb_port",
    pcb_port_id: "pcb_port_a",
    source_port_id: "source_port_a",
    x: -5,
    y: 0,
    layers: ["top"],
  },
  {
    type: "pcb_port",
    pcb_port_id: "pcb_port_b",
    source_port_id: "source_port_b",
    x: 5,
    y: 0,
    layers: ["top"],
  },
  {
    type: "pcb_port",
    pcb_port_id: "pcb_port_c",
    source_port_id: "source_port_c",
    x: 0,
    y: 5,
    layers: ["top"],
  },
  {
    type: "pcb_smtpad",
    pcb_smtpad_id: "pcb_smtpad_a",
    pcb_port_id: "pcb_port_a",
    shape: "rect",
    x: -5,
    y: 0,
    width: 1,
    height: 1,
    layer: "top",
  },
  {
    type: "pcb_smtpad",
    pcb_smtpad_id: "pcb_smtpad_b",
    pcb_port_id: "pcb_port_b",
    shape: "rect",
    x: 5,
    y: 0,
    width: 1,
    height: 1,
    layer: "top",
  },
  {
    type: "pcb_smtpad",
    pcb_smtpad_id: "pcb_smtpad_c",
    pcb_port_id: "pcb_port_c",
    shape: "rect",
    x: 0,
    y: 5,
    width: 1,
    height: 1,
    layer: "top",
  },
  // trace1: pad_A → junction (has source_trace_id)
  {
    type: "pcb_trace",
    pcb_trace_id: "pcb_trace_1",
    source_trace_id: "source_trace_1",
    route: [
      {
        route_type: "wire",
        x: -5,
        y: 0,
        layer: "top",
        width: 0.2,
        start_pcb_port_id: "pcb_port_a",
      },
      { route_type: "wire", x: 0, y: 0, layer: "top", width: 0.2 },
    ],
  },
  // trace2: junction → pad_B (NO source_trace_id — branching trace)
  {
    type: "pcb_trace",
    pcb_trace_id: "pcb_trace_2",
    route: [
      { route_type: "wire", x: 0, y: 0, layer: "top", width: 0.2 },
      {
        route_type: "wire",
        x: 5,
        y: 0,
        layer: "top",
        width: 0.2,
        end_pcb_port_id: "pcb_port_b",
      },
    ],
  },
  // trace3: junction → pad_C (NO source_trace_id — branching trace)
  {
    type: "pcb_trace",
    pcb_trace_id: "pcb_trace_3",
    route: [
      { route_type: "wire", x: 0, y: 0, layer: "top", width: 0.2 },
      {
        route_type: "wire",
        x: 0,
        y: 5,
        layer: "top",
        width: 0.2,
        end_pcb_port_id: "pcb_port_c",
      },
    ],
  },
] as AnyCircuitElement[]

test("branching traces should not report false contiguity errors", () => {
  const traceErrors = checkTracesAreContiguous(circuitJson)

  expect(
    convertCircuitJsonToPcbSvg([...circuitJson, ...traceErrors], {
      shouldDrawErrors: true,
      showErrorsInTextOverlay: true,
    }),
  ).toMatchSvgSnapshot(import.meta.path)

  // BUG: should be 0 — these are false positives from branching traces
  // without source_trace_id not being recognized as connected
  expect(traceErrors).toHaveLength(4)
})
