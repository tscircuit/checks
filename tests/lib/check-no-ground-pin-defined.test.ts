...
describe("checkNoGroundPinDefined", () => {
  test("returns warning when chip has no requires_ground pin", () => {
    ...
    const warnings = checkNoGroundPinDefined(circuitJson)
    expect(warnings).toHaveLength(1)
    expect(warnings[0].type).toBe("source_no_ground_pin_defined_warning")
  ...
  test("returns no warning when at least one pin has requires_ground=true", () => {
    const circuitJson: AnyCircuitElement[] = [
      {
        type: "source_component",
        source_component_id: "component_1",
