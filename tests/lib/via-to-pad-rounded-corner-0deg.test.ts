...
const errors = (cornerRadius: number, d: number) =>
  checkViaPadClearance([pad(cornerRadius), obstacle(d)], { minClearance: 0.5 })

test("snapshots via-to-pad rounded-corner clearance at 0 degrees", () => {
  ...
  expect(svg).toMatchSvgSnapshot(
    import.meta.path,
    "via-to-pad-rounded-corner-0deg",
  )
})

test.failing("accepts the legal via-to-pad gap at 0 degrees", () => {
...
test("rejects the sharp-corner via-to-pad gap at 0 degrees", () => {
